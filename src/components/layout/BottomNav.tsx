...
export const BottomNav: React.FC = () => {
  ...
  const renderNavItem = (item: any) => (
    <NavLink
      ...
      to={item.path}
      className={({ isActive }) =>
        `flex flex-col items-center justify-start pt-3 transition-all flex-1 max-w-[76px] ${
      ...
      {({ isActive }) => (
        <>
          <item.icon 
            ...
            className={`w-[20px] h-[20px] mb-1.5 ${
  ...
  return (
    ...
    >
      <div className="relative w-full h-[72px] mx-auto flex items-end justify-center pointer-events-auto">
        ...
        >
          {/* Left Side */}
          <div className="flex-1 bg-white rounded-tr-[4px]"></div>
          
          {/* Smooth Notch */}
          <div className="w-[84px] h-full relative shrink-0">
            <svg 
              viewBox="0 0 84 72" 
              preserveAspectRatio="none"
              className="w-full h-full text-white fill-current"
            >
              <path d="M0 0 C 18 0, 22 42, 42 42 C 62 42, 66 0, 84 0 L 84 72 L 0 72 Z" />
            </svg>
          </div>
          
          {/* Right Side */}
          <div className="flex-1 bg-white rounded-tl-[4px]"></div>
        </div>
        ...
        <div className="relative z-10 flex items-start justify-around w-full max-w-[420px] px-2 h-full pt-0.5">
          ...
          <div className="relative flex flex-col items-center -mt-[26px] w-[72px]">
            <NavLink 
              to="/jobs"
              className={`flex items-center justify-center w-[54px] h-[54px] rounded-full shadow-lg transition-transform active:scale-95 ${
                isSearchActive ? 'bg-blue-700 text-white shadow-blue-500/30' : 'bg-blue-600 text-white'
              }`}
            >
